<script lang="ts">
  ...
  $: if ($userRepos) {
    allRepos = Object.values($userRepos).map(updateProjectMetaData);
  ...
  $: settingsDescription = getFilterSettingsDescriptin(
    filtered,
    language,
    category
  );

  function getLanguageFilterSettings(
  ...
  function getCategoryFilterSettings(
    projects: RepoWithMetaData[]
  ): FilterSetting[] {
  ...
  function getFilterSettingsDescriptin(
    ...
    const settings = [langSetting, catSetting]
      .filter((s) => s !== null)
      .join(", ");
...
<div class="project-list-wrapper">
  ...
  {:else}
    <div class="zero-results">
      None of my projects match the current filter settings!
    </div>
  {/if}
</div>
