...
<section
  class="mt-4 mb-16 w-full border-y border-gray-700 p-6 dark:border-gray-500 sm:rounded sm:border-x"
  id="newsletter"
>
  <div
    class="flex items-center justify-between space-x-4 text-gray-900 dark:text-gray-100"
  >
  ...
  {#if isNewsletterOpen}
    ...
    <form class="relative my-4" on:submit={onSubmit}>
      <input
        ...
        autocomplete="email"
        required={true}
        class="mt-1 block w-full rounded-md px-4 py-2 pr-32 border-gray-300  text-gray-900 focus:border-blue-500 focus:ring-blue-500 dark:text-gray-100"
      />
      <button
        class="absolute right-1 top-1 flex h-8 w-28 items-center justify-center tracking-wide rounded px-4 pt-1 text-sm font-medium text-gray-900 dark:text-gray-100"
