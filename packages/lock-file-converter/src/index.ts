...
export async function createMonorepoLockFile(): Promise<void> {
	...
	await writePackageLock(lock, MONOREPO_ROOT);
}
...
async function dedicatedLockfile(lockfileDir: string, projectDir: string): Promise<Lockfile> {
...
async function convertPnpmLockFile(lock: Lockfile, lockfileDir: string): Promise<PackageLock> {
	...
	const { name, version } = await readProjectManifestOnly(lockfileDir).then(
		({ name, version }) => {
			if (!name)
				console.warn("Package name not found in manifest, using placeholder value ('package').");
			if (!version)
				console.warn("Package version not found in manifest, using placeholder value ('0.0.0').");
	...
	return {
		...
		lockfileVersion: 1,
		requires: true,
		dependencies: dependenciesFromNamedDependencies(
			namedDependenciesFromSnapshots(lock.packages ?? {})
		)
	};
}

function namedDependenciesFromSnapshots(
	snapshots: PackageSnapshots
): Record<string, NamedDependency> {
	const missingRootDepPaths = missingDepPaths(snapshots);

	function namedDependencyFromSnapshot([depPath, snapshot]: [string, PackageSnapshot]): [
		string,
		NamedDependency
	] {
		const { name, version } = nameVerFromPkgSnapshot(depPath, snapshot as PackageSnapshot);
		const resolution = pkgSnapshotToResolution(depPath, snapshot, {
...
function dependenciesFromNamedDependencies(
	namedDependencies: Record<string, NamedDependency>
